...
class Arrow:
    ...
    def __init__(self, start, end, short_render, text: str, color: str, start_pad):
        ...
        pm.draw_text(text=text, posX=head_v3.x + 20, posY=head_v2.y - 5, fontSize=9, color=pm_colors["orange"])
        pm.draw_triangle(head_v1.x, head_v1.y, head_v2.x, head_v2.y, head_v3.x, head_v3.y, color=pm_colors[color])
        ...
        for p in range(1, 13):
            ...
            if len(cross_color) == 0:
                pm.draw_line(start_point.x, start_point.y, end_point.x, end_point.y,
                             color=pm_colors["white"], thick=1)
            elif len(cross_color) == 1:
                pm.draw_line(start_point.x, start_point.y, end_point.x, end_point.y,
                             color=pm_colors[cross_color[0]], thick=1)
            else:
                pm.draw_line(start_point.x, start_point.y, end_point.x, end_point.y,
                             color=pm_colors[cross_color[0]] if p < 6 else pm_colors[cross_color[1]], thick=1)
...
class PlayerLabel:
    ...
    def __init__(self, position, text, size, scale_y, font_size, text_color, back_color):
        ...
        pm.draw_font(1, text, start_point_text.x + 4, start_point_text.y, font_size, 0, pm_colors[text_color])
        pm.draw_rectangle_rounded(start_point_text.x, start_point_text.y, text_length, font_size, 0.8, 1, pm_colors[back_color])
