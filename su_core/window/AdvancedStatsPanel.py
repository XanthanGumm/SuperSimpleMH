...
class AdvancedStatsPanel:
    ...
    def draw_advanced_stats(self):
        ...
        if self._stats_tooltips:
            pm.draw_texture(self._advanced_stats_texture, 0, self._height_pad, pm_colors["white"], 0, 1)

            for i in range(len(self._stats_tooltips)):
                pm.draw_texture(
                    self._value_box_texture, self._value_box_x, self._value_box_y, pm_colors["white"], 0, 1
                )
                ...
                if text_len["x"] >= line_limit - 1:
                    ...
                    pm.draw_font(2, text_1, text_1_x, text_1_y, self._font_size, 0, pm_colors["white"])
                    pm.draw_font(2, text_2, text_2_x, text_2_y, self._font_size, 0, pm_colors["white"])
