...
class InventoryPanel:
    def __init__(self, win_width, win_height, win_start_x, win_start_y, font_size):
        ...
        with open(
            os.path.join(sprites_path, "inventory_coords.toml"), "rb"
        ) as inv_file:
        ...
        self._switch1_y = (
            self._scale_h * self._coords["switch1"]["ymin"] + self._height_pad
        )
        self._switch2_x = self._scale_w * self._coords["switch2"]["xmin"]
        self._switch2_y = (
            self._scale_h * self._coords["switch2"]["ymin"] + self._height_pad
        )
        ...
        for i in range(1, 3):
            ...
            img_switch = img_switch.resize(
                (
                    int(self._scale_w * img_switch.size[0]),
        ...
        # load items textures
        for cur_root, _, files in os.walk(
            os.path.join(root, "resources", "sprites", "items")
        ):
            ...
            for file in files:
                ...
                item_img = item_img.resize(
                    (
                        math.ceil(item_img.size[0] * self._scale_w),
                        math.ceil(item_img.size[1] * self._scale_h),
                    )
                )
                img_byte_arr = io.BytesIO()
                item_img.save(img_byte_arr, format="PNG")
                item_img = img_byte_arr.getvalue()
                self._item_textures[cur_dir][item_name] = pm.load_texture_bytes(
                    ".png", item_img
                )
    ...
    def create_tooltips(self):
        ...
        if self._hover_player is not None:
            ...
            if self._hover_player.inventory.helm is not None:
                self._tooltips[
                    "helm"
                ] = self._hover_player.inventory.helm.create_tooltip(player_level)

            if self._hover_player.inventory.amulet is not None:
                self._tooltips[
                    "amulet"
                ] = self._hover_player.inventory.amulet.create_tooltip(player_level)

            if self._hover_player.inventory.armor is not None:
                self._tooltips[
                    "armor"
                ] = self._hover_player.inventory.armor.create_tooltip(player_level)

            if self._hover_player.inventory.arm_left is not None:
                self._tooltips[
                    "arm_left"
                ] = self._hover_player.inventory.arm_left.create_tooltip(player_level)

            if self._hover_player.inventory.arm_right is not None:
                self._tooltips[
                    "arm_right"
                ] = self._hover_player.inventory.arm_right.create_tooltip(player_level)

            if self._hover_player.inventory.ring_left is not None:
                self._tooltips[
                    "ring_left"
                ] = self._hover_player.inventory.ring_left.create_tooltip(player_level)

            if self._hover_player.inventory.ring_right is not None:
                self._tooltips[
                    "ring_right"
                ] = self._hover_player.inventory.ring_right.create_tooltip(player_level)

            if self._hover_player.inventory.belt is not None:
                self._tooltips[
                    "belt"
                ] = self._hover_player.inventory.belt.create_tooltip(player_level)

            if self._hover_player.inventory.boots is not None:
                self._tooltips[
                    "boots"
                ] = self._hover_player.inventory.boots.create_tooltip(player_level)

            if self._hover_player.inventory.gloves is not None:
                self._tooltips[
                    "gloves"
                ] = self._hover_player.inventory.gloves.create_tooltip(player_level)

            if self._hover_player.inventory.arm_switch_left is not None:
                self._tooltips[
                    "arm_switch_left"
                ] = self._hover_player.inventory.arm_switch_left.create_tooltip(
                    player_level
                )
            ...
            if self._hover_player.inventory.arm_switch_right is not None:
                self._tooltips[
                    "arm_switch_right"
                ] = self._hover_player.inventory.arm_switch_right.create_tooltip(
                    player_level
                )
            ...
            for i in range(10):
                for j in range(4):
                    ...
                    if grid_item is not None:
                        ...
                        item_size = (
                            2
                            if item_type == "LargeCharm"
                            else 3
                            if item_type != "SmallCharm"
                            else 1
                        )
                        xmin = x + i * w
                        xmax = xmin + w
    ...
    def draw_inventory(self):
        ...
        switch_texture = (
            self._switch_textures[0]
            if not self._is_on_switch
            else self._switch_textures[1]
        )
        pm.draw_texture(
            switch_texture, self._switch1_x, self._switch1_y, pm_colors["white"], 0, 1
        )
        pm.draw_texture(
            switch_texture, self._switch2_x, self._switch2_y, pm_colors["white"], 0, 1
        )
    ...
    def draw_item_tooltip(self):
        if (
            self._is_loc_hovered("helm")
            and self._hover_player.inventory.helm is not None
        ):
            x, y, w, h = self._inv_loc_position("helm")
            self._draw_item_tooltip("helm", x + w // 2, y + h + self._height_pad)
        elif (
            self._is_loc_hovered("amulet")
            and self._hover_player.inventory.amulet is not None
        ):
            x, y, w, h = self._inv_loc_position("amulet")
            self._draw_item_tooltip("amulet", x + w // 2, y + h + self._height_pad)
        elif (
            self._is_loc_hovered("armor")
            and self._hover_player.inventory.armor is not None
        ):
            ...
            if (
                self._is_on_switch
                and self._hover_player.inventory.arm_switch_left is not None
            ):
                self._draw_item_tooltip(
                    "arm_switch_left", x + w // 2, y + h + self._height_pad
                )
            if (
                not self._is_on_switch
                and self._hover_player.inventory.arm_left is not None
            ):
                self._draw_item_tooltip(
                    "arm_left", x + w // 2, y + h + self._height_pad
                )
        elif self._is_loc_hovered("arm_right"):
            x, y, w, h = self._inv_loc_position("arm_right")
            if (
                self._is_on_switch
                and self._hover_player.inventory.arm_switch_right is not None
            ):
                self._draw_item_tooltip(
                    "arm_switch_right", x + w // 2, y + h + self._height_pad
                )
            if not self._is_on_switch and self._hover_player.inventory.arm_right:
                self._draw_item_tooltip(
                    "arm_right", x + w // 2, y + h + self._height_pad
                )
        elif (
            self._is_loc_hovered("ring_left")
            and self._hover_player.inventory.ring_left is not None
        ):
            x, y, w, h = self._inv_loc_position("ring_left")
            self._draw_item_tooltip(
                "ring_left", x + w // 2, y + self._height_pad, direction="up"
            )
        elif (
            self._is_loc_hovered("ring_right")
            and self._hover_player.inventory.ring_right is not None
        ):
            x, y, w, h = self._inv_loc_position("ring_right")
            self._draw_item_tooltip(
                "ring_right", x + w // 2, y + self._height_pad, direction="up"
            )
        elif (
            self._is_loc_hovered("belt")
            and self._hover_player.inventory.belt is not None
        ):
            x, y, w, h = self._inv_loc_position("belt")
            self._draw_item_tooltip(
                "belt", x + w // 2, y + self._height_pad, direction="up"
            )
        elif (
            self._is_loc_hovered("boots")
            and self._hover_player.inventory.boots is not None
        ):
            x, y, w, h = self._inv_loc_position("boots")
            self._draw_item_tooltip(
                "boots", x + w // 2, y + self._height_pad, direction="up"
            )
        elif (
            self._is_loc_hovered("gloves")
            and self._hover_player.inventory.gloves is not None
        ):
            x, y, w, h = self._inv_loc_position("gloves")
            self._draw_item_tooltip(
                "gloves", x + w // 2, y + self._height_pad, direction="up"
            )
            ...
            for j in range(4):
                ...
                if item_key in self._tooltips:
                    ...
                    if self._is_loc_hovered(item_key):
                        self._draw_item_tooltip(
                            item_key, x + w // 2, y + self._height_pad, direction="up"
                        )
    ...
    def _draw_inv_item(self, loc: str):
        try:
            ...
            if self._is_loc_hovered(loc):
                pm.draw_rectangle(
                    x, y + self._height_pad, w, h, pm_colors["invbackground"]
                )
            pm.draw_texture(
    ...
    def _draw_item_tooltip(self, loc, start_x, start_y, direction="down"):
        ...
        if type_pad_x is not None and type_pad_y is not None:
            pm.draw_font(
                ...
                self._font_size,
                0,
                pm_colors["tooltipgray"]
                if item_quality == "RUNEWORD"
                else pm_colors[color],
            )
    ...
    def _is_loc_hovered(self, loc: str) -> bool:
        ...
        relative_mouse = {
            "x": absolute_mouse["x"] - self._win_start_x,
            "y": absolute_mouse["y"] - self._win_start_y,
        }
        x, y, w, h = self._inv_loc_position(loc)
        y += self._height_pad
